      ...
      try {
        ...
        const job = await res.json()
          ...
          setStatus(prev => ({
            ...
            failures: job.error_log || []
          }))


        if (job.status === "completed") {
          clearInterval(interval)
          toast.success(`Import completed! ${job.matched_items} series added.`)
  ...
  return (
    <div className="space-y-4">
      ...
      {status.status === "completed" && (
        <div className="bg-green-50 dark:bg-green-950/20 border border-green-100 dark:border-green-900/30 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-green-700 dark:text-green-500/80 mt-1">
              ...
              <Button 
                variant="link" 
                className="p-0 h-auto text-xs font-bold text-green-700 dark:text-green-500 mt-2"
            ...
            {status.failures && status.failures.length > 0 && (
              ...
              >
                {showFailures ? "Hide" : "Review"} {status.failures.length} skipped
              </Button>
            )}
        ...
        {status.status === "completed" && showFailures && status.failures && status.failures.length > 0 && (
          <div className="bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden animate-in fade-in slide-in-from-top-2">
            <div className="p-3 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-800/50 flex justify-between items-center">
              <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Skipped Series Review</span>
              <span className="text-[10px] text-zinc-400">{status.failures.length} items</span>
            </div>
            <div className="max-h-[240px] overflow-y-auto">
              {status.failures.map((fail, i) => (
                <div key={i} className="p-3 flex items-start gap-3 border-b border-zinc-100 dark:border-zinc-800 last:border-0 hover:bg-white dark:hover:bg-zinc-800 transition-colors">
                  <div className="size-6 rounded-lg bg-zinc-200 dark:bg-zinc-700 flex items-center justify-center shrink-0">
                    <FileText className="size-3 text-zinc-500" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs font-bold truncate">{fail.title}</p>
                    <p className="text-[10px] text-zinc-500 mt-0.5">{fail.reason}</p>
                  </div>
                </div>
              ))}
            </div>
